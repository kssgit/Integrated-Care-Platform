...
class MixedQualityExtractor(Extractor[dict]):
    async def extract(self) -> list[dict]:
        return [
            {
                "source_id": "A1",
...
@pytest.mark.asyncio
async def test_pipeline_quality_gate_filters_invalid_record_when_threshold_allows() -> None:
    store = StubStore()
    pipeline = ETLPipeline(
        extractor=MixedQualityExtractor(),
        store=store,
        quality_gate=FacilityQualityGate(max_reject_ratio=0.5),
    )
